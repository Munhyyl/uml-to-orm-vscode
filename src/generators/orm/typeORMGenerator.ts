...
export class TypeORMGenerator implements CodeGenerator {
  async generate(schema: ProjectSchema): Promise<string> {
    ...
    for (const entity of schema.entities) {
      ...
      output += this.generateEntityClass(entity, schema);
      output += '\n\n';
    }
  ...
  private generateEntityClass(entity: ClassEntity, schema: ProjectSchema): string {
  ...
  private generateMethod(method: Method): string {
    ...
    let code = `  ${vis}${stat}${method.name}(${params}): ${returnType} {\n`;
    code += `    // TODO: implement\n`;
